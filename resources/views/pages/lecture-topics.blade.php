<x-layouts.app>
    <main class="flex-grow py-8 max-w-5xl mx-auto w-full px-4">
        ...
        <div class="mb-8">
            ...
            <span class="mx-2 text-gray-300 dark:text-gray-600">&rsaquo;</span>
            <span class="text-brand-primary text-sm font-bold">Recorded Lectures</span>
        </div>
        ...
        <h1 class="text-3xl md:text-4xl font-heading font-extrabold text-gray-900 dark:text-white mb-8">
            {{ $subject->name }} &mdash; Recorded Lectures
        </h1>

        {{-- Topic list — always shown, with per-topic badge --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse($topics as $topic)
                <a href="{{ route('subjects.lectures.show', [$subject->slug, $topic->slug]) }}"
                   class="group glass-card rounded-2xl p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-heading font-bold text-gray-900 dark:text-white group-hover:text-brand-secondary dark:group-hover:text-purple-400 transition-colors">
                            {{ $topic->name }}
                        </h3>
                        @if($topic->recorded_lectures_count > 0)
                            <span class="badge bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 mt-1">
                                {{ $topic->recorded_lectures_count }} lecture(s)
                            </span>
                        @else
                    ...
                    <svg class="w-5 h-5 text-gray-300 dark:text-gray-600 group-hover:text-brand-secondary dark:group-hover:text-purple-400 group-hover:translate-x-1 transition-all shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
